...
async function getListByPage(ctx) {
    ...
    let sortKey = ctx.request.query.sort ? (ctx.request.query.sort + ((ctx.request.query.sort.indexOf('asc') > -1 || ctx.request.query.sort.indexOf('desc') > -1) ? '' : ' desc')) : 'sequence desc'
    const params = [];
    let sql = '';
    if (keys.length > 0) {
        sql += ' where';
